...
function cleanSideEffectPoint(text){
  return String(text||"")
    .replace(/\s+/g," ")
    .replace(/^[\s?â‚¬?\-.?â‚¬â€œ?â‚¬â€*]+/g,"")
    .replace(/^(?:[\s?â‚¬?\-.?â‚¬â€œ?â‚¬â€*]+\s*)+/g,"")
    .replace(/\s+([,.;:])/g,"$1")
    .trim();
}
